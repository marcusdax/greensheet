# 02 — ALT-001 → ALT-005 Campaign Expansion (Complete Nurture Engine)
...
|---|---|---|---|---|---|
| **ALT-001** | *First Crack* | Activate new lead → sample kit request | `lead.qualified` (lead_score ≥ 55) | `sample_kit.requested` | Kit requested, or 3 touches w/o engagement |
| **ALT-002** | *The Cupping* | Kit delivered → cupping feedback | `sample_kit.delivered` + 4 days | `feedback.submitted` | Feedback submitted |
| **ALT-003** | *The Shortlist* | Feedback → first paid order | `feedback.submitted` | `order.created` (first) | Order created (`EXECUTE_CAMPAIGN_HALT`) |
| **ALT-004** | *Second Cup* | Rescue non-responders | `sample_kit.delivered` + 9 days, no feedback | `feedback.submitted` (late) | Feedback or day-21 suppression |
| **ALT-005** | *The Regular* | First order → reorder + referral | `order.delivered` + 14 days | 2nd order ≤ 90d; referral invite sent | Reorder logged; referral invite accepted |
...
## ALT-001 — *First Crack* (Lead → Sample Kit Request)
...
**Audience segment:** New roaster leads with `lead_score ≥ 55` (XGBoost score from firmographic + behavioral history, §I.2). Copy variants for `segment='micro'` (shown here) and `segment='boutique'` (noted inline). Commercial/enterprise leads route to sales-assisted track, not ALT-001.
...
### Email — Touch 1 (`template_id: ALT-001-E1`, `touchpoint: 1`, `template_style: option_a_sensory`)
...
### Email — Touch 2 (`ALT-001-E2`, `touchpoint: 2`, fires day 3 if unopened)
...
### SMS — Touch 3 (`ALT-001-S1`, `touchpoint: 3`, `template_style: option_b_consultative`, fires day 5 if opened-but-unconverted)
...
### Landing page (`/kits/claim?utm_campaign=alt-001`)
...
| LP conversion | claims ÷ LP sessions (UTM `alt-001`) | — | ≥ 38% | +5pp |
  ...
  "rule_actions": [
    {
      ...
      "template_id": "ALT-001-E1",
      "channel_type": "email",
      "touchpoint": 1,
      "delay_minutes": 0,
      "ab_test_id": "abt_alt001_subject_v1"
    },
    {
      "action_type": "SEND_EMAIL",
      "template_id": "ALT-001-E2",
      "channel_type": "email",
      "touchpoint": 2,
      "delay_minutes": 4320,
    ...
    },
    {
      ...
      "payload": {
        ...
        "halt_scope": "campaign_id:ALT-001",
...
## ALT-002 — *The Cupping* (Kit Delivered → Feedback Submitted)
...
### Email — Touch 1 (`ALT-002-E1`, `touchpoint: 1`, `template_style: option_a_sensory`)
...
### SMS — Touch 2 (`ALT-002-S1`, `touchpoint: 2`, `template_style: option_b_consultative`, day 7 if no feedback)
...
### Landing page (`/feedback?kit={kit_id}&utm_campaign=alt-002`)
...
- **Post-submit state:** instant personalized shortlist → routes into ALT-003 within 2 hours.
...
```json
{
  ...
  "rule_actions": [
    {
      "action_type": "SEND_EMAIL",
      "template_id": "ALT-002-E1",
      "channel_type": "email",
      "touchpoint": 1,
      "delay_minutes": 0,
      "ab_test_id": "abt_alt002_subject_v1"
    },
    {
      "action_type": "SEND_SMS",
      "template_id": "ALT-002-S1",
      "channel_type": "sms",
      "touchpoint": 2,
      "delay_minutes": 4320,
    ...
    },
    {
      ...
      "payload": {
        ...
        "halt_scope": "campaign_id:ALT-002",
        "post_halt_route": { "feedback.submitted": "campaign:ALT-003", "order.created": "campaign:ALT-005" },
...
## ALT-003 — *The Shortlist* (Feedback → First Paid Order)
...
**Trigger events:** `feedback.submitted` → touch 1 within 2 hours (shortlist is freshest). No order by day 5 → touch 2 (social proof + scarcity, real counts only). No order by day 10 → touch 3 (SMS, consultative close). Any `order.created` → `EXECUTE_CAMPAIGN_HALT` immediately, route to ALT-005.
...
### Email — Touch 1 (`ALT-003-E1`, `touchpoint: 1`, fires ≤ 2 h after feedback)
...
### Email — Touch 2 (`ALT-003-E2`, `touchpoint: 2`, day 5, no order)
...
### SMS — Touch 3 (`ALT-003-S1`, `touchpoint: 3`, day 10, no order)
...
### Landing page (`/shortlist?utm_campaign=alt-003`)
  ...
  "rule_actions": [
    ...
    },
    {
      ...
      "payload": {
        ...
        "halt_scope": "campaign_id:ALT-003",
        "post_halt_route": { "order.created": "campaign:ALT-005" },
...
## ALT-004 — *Second Cup* (Non-Responder Rescue)
...
**Audience segment:** Kit delivered ≥ 9 days, no `feedback.submitted`, no order. Diagnosis branches: (a) never opened ALT-002 (deliverability/attention problem → new angle, new sender name); (b) opened but didn't act (motivation problem → lower the ask).
...
### Email — Touch 1 (`ALT-004-E1`, `touchpoint: 1`, day 9)
...
### SMS — Touch 2 (`ALT-004-S1`, `touchpoint: 2`, day 14)
...
### Landing page (`/second-cup?utm_campaign=alt-004`)
...
```json
{
  ...
  "conditions_json": {
    ...
    "suppression_check": {
      "unsubscribed": false,
      "active_campaign_not_in": ["ALT-003"],
  ...
  "rule_actions": [
    {
      "action_type": "SEND_EMAIL",
      "template_id": "ALT-004-E1",
      "channel_type": "email",
      "touchpoint": 1,
      "delay_minutes": 0,
      "ab_test_id": "abt_alt004_subject_v1"
    },
    {
      "action_type": "SEND_SMS",
      "template_id": "ALT-004-S1",
      "channel_type": "sms",
      "touchpoint": 2,
      "delay_minutes": 7200,
    ...
    },
    {
      ...
      "payload": {
        ...
        "halt_scope": "campaign_id:ALT-004",
        "post_halt_route": { "feedback.submitted": "campaign:ALT-003", "order.created": "campaign:ALT-005" },
...
## ALT-005 — *The Regular* (First Order → Reorder + Referral Seed)
...
### Email — Touch 1 (referral seed, `ALT-005-E1`, `touchpoint: 1`, day 14)
...
### Email — Touch 2 (reorder, `ALT-005-E2`, `touchpoint: 2`, day 45)
...
### SMS — Touch 3 (`ALT-005-S1`, `touchpoint: 3`, day 52 if no reorder)
...
### Landing page (`/reorder?utm_campaign=alt-005`)
...
```json
{
  ...
  "rule_actions": [
    {
      "action_type": "SEND_EMAIL",
      "template_id": "ALT-005-E1",
      "channel_type": "email",
      "touchpoint": 1,
      "delay_minutes": 0,
      "ab_test_id": "abt_alt005_subject_v1",
      "personalization": { "tokens": ["{referral_url}", "{origin}", "{process_method}"] }
    },
    {
      "action_type": "SEND_EMAIL",
      "template_id": "ALT-005-E2",
      "channel_type": "email",
      "touchpoint": 2,
      "delay_minutes": 44640,
    ...
    },
    {
      ...
      "payload": {
        "halt_on_event": ["order.created", "user.unsubscribed"],
        "halt_scope": "campaign_id:ALT-005",
...
Per 1,000 qualified leads entering ALT-001, at target rates:

| Stage | Rate | Count | Cumulative cost | Cumulative expected yr-1 contribution |
|---|---|---|---|---|
| Qualified leads | — | 1,000 | $12k (lead-gen allocation) | — |
| Kit requests (ALT-001) | 32% | 320 | + $12.2k kits | — |
| Feedback (ALT-002 + late ALT-004) | 45% + 9% of remainder | 144 + 16 = 160 | — | — |
| First orders (ALT-003) | 40% of feedback | 64 | — | 64 × $2,100 = $134.4k |
| 90-day reorders (ALT-005) | 55% of first orders | 35 | — | (drives yr-2 retention base) |
...
## 7. Global suppression, logging & compliance rules (apply to all ALT campaigns)
...
```json
{
  ...
  "rule_actions": [
    ...
    { "action_type": "UPDATE_CRM_LIFECYCLE", "payload": { "on_unsubscribe": { "lifecycle_stage": "suppressed", "newsletter_only": true }, "on_order_created": { "route": "campaign:ALT-005" } } }
  ]
}
```
