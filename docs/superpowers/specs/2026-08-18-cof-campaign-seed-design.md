# ALT-001 → ALT-005 Campaign Seed Data — Design

## Goal

Replace the placeholder campaign/rule seed data in the Auctum Ledger app with the complete ALT-001 → ALT-005 nurture-engine definitions from `marketing/02-cof-campaign-expansion.md`, so the existing CampaignsPage and automation-rule layer display real campaign definitions, templates, merge tokens, and A/B variants.

## Scope

**In scope:**
- Add marketing-specific types (`MarketingTemplate`, `CampaignToken`).
- Add a canonical merge-token registry and a template library covering every email/SMS touch in ALT-001..005.
- Seed `db.campaigns`, `db.rules`, and `db.templates` with accurate ALT data.
...
| `app/src/api/marketing-data.ts` | Constants `CAMPAIGN_TOKENS` (18 canonical tokens from §0.2 plus 6 extended copy tokens) and `MARKETING_TEMPLATES` (all 12 ALT email/SMS templates with A/B subjects and body copy). |
...
| `app/src/api/client.ts` | Update `api.campaigns.performance` to return metrics that match each ALT campaign's primary metric and funnel. |
...
- **Actions:**
  1. Send `ALT-001-E1` email immediately (A/B subject).
  2. Send `ALT-001-E2` email after 4,320 min (3 days) if unopened.
  3. Send `ALT-001-S1` SMS after 7,200 min (5 days) if opened but no kit request.
...
- **Actions:**
  1. Send `ALT-002-E1` email.
  2. Send `ALT-002-S1` SMS after 4,320 min (3 days) if no feedback.
  3. Update lifecycle to `kit_cupping_window`.
  4. Halt on `feedback.submitted`, `order.created`, or `user.unsubscribed`; route to ALT-003 or ALT-005.

### ALT-003 — The Shortlist
- **Trigger:** `feedback.submitted`
- **Actions:**
  1. Send `ALT-003-E1` email after 120 min (2 hours), branching on feedback valence.
  2. Send `ALT-003-E2` email after 7,200 min (5 days) if no order.
  3. Send `ALT-003-S1` SMS after 14,400 min (10 days) if no order.
  4. Update lifecycle to `shortlist_presented`.
  5. Halt on `order.created` or `user.unsubscribed`; route to ALT-005.

### ALT-004 — Second Cup
- **Trigger:** `sample_kit.delivered` + 9 days, no feedback, no order
- **Actions:**
  1. Send `ALT-004-E1` email.
  2. Send `ALT-004-S1` SMS after 7,200 min (5 days) if no structured response.
  3. Schedule exit to quarterly newsletter at day 21 with lead-score decay.
  4. Halt on `feedback.submitted`, `order.created`, `structured_response.logged`, or `user.unsubscribed`; route to ALT-003 or ALT-005.

### ALT-005 — The Regular
- **Trigger:** `order.delivered` + 14 days, first order
- **Actions:**
  1. Send `ALT-005-E1` referral email.
  2. Send `ALT-005-E2` reorder email after 44,640 min (31 days) if no reorder.
  3. Send `ALT-005-S1` SMS after 54,720 min (38 days) if no reorder and previous click.
  4. Update lifecycle to `first_order_active`.
  5. Halt on `order.created` or `user.unsubscribed`.

## Schema mapping notes

The marketing spec uses `SEND_EMAIL` / `SEND_SMS` action types and rule IDs such as `ALT-001-R1`. The app's existing `RuleActionType` enum only defines `SEND_TEMPLATE`, plus `UPDATE_CRM_LIFECYCLE` and `EXECUTE_CAMPAIGN_HALT`. To keep seed data compatible with the existing API/types without expanding the runtime schema, rule actions will:
- Use `SEND_TEMPLATE` with `channel: 'email'` or `channel: 'sms'` for all sends.
- Store `ab_test_id` and `fire_if` inside the action `payload`.
- Keep `ruleCode` values as `ALT-001` … `ALT-005` to match the existing `^ALT-00[1-9]$` validation and the campaign codes.
...
- ALT-001: funnel `kitSent → opened → clicked → requested`
- ALT-002: funnel `kitSent → opened → feedbackSubmitted`
- ALT-003: funnel `feedbackSubmitted → opened → ordered`
- ALT-004: funnel `kitSent → opened → responded`
- ALT-005: funnel `firstOrders → referralSent → reordered`
...
| `RuleForm` ruleCode regex limits to `ALT-00[1-9]` | New rule codes stay within this pattern. |
| Large markdown copy in code | Store templates in a dedicated data file, not inline in components. |

## Success criteria

- `npm run test:run` passes.
- CampaignsPage loads and shows five ALT campaigns with correct names, triggers, and rule sequences.
