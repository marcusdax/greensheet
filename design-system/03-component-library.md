...
Conventions used below: React 18 + Tailwind (config per §10 of the tokens doc) + framer-motion. Class names reference the Auctum Ledger (formerly Greensheet) Tailwind extension (`bg-surface`, `text-ink`, `shadow-e3`, `duration-base`, `ease-seal`, …). No arbitrary hex values anywhere. Every interactive element ships **default / hover / focus / active / disabled / loading** states and, where applicable, **empty / error** states.

Shared state rules:

- **Focus:** 2 px `border-focus` ring + 2 px offset (`focus-visible:ring-2 ring-oxblood ring-offset-2 ring-offset-canvas`). Never `outline: none` without the ring replacement.
...
- **Loading:** spinner = 20 px ring, 2 px stroke, `oxblood-700` arc on `recessed` track, 750 ms linear rotation; buttons show spinner + preserve width (label swaps to spinner, `aria-busy`).
...
|---|---|---|
| `primary` | `oxblood-700` bg, `#FAF9F4` text (8.62:1 AAA) | one per view — the forward action ("Source This Lot", "Send Campaign") |
| `secondary` | `ink-900` bg, `#FAF9F4` text (15.69:1 AAA) | supporting action ("Save View", "Export Sheet") |
...
| `brass` | `brass-300` bg, `ink-900` text (7.32:1 AAA) | **reserved**: moments of earned value — claim offer, publish winner. Max one per flow |
| `destructive` | `danger` bg, white text (7.80:1 AAA) | irreversible actions, always with confirm step |
...
|---|---|---|---|
| Hover | `oxblood-800`, shadow `e2` | `ink-700` | bg `recessed` |
...
const variants = {
  primary:     'bg-oxblood text-paper-50 hover:bg-oxblood-800 shadow-e1 hover:shadow-e2',
  secondary:   'bg-ink-900 text-paper-50 hover:bg-ink-700 shadow-e1 hover:shadow-e2',
  ...
  brass:       'bg-brass-300 text-ink hover:bg-brass-500 shadow-e1 hover:shadow-e2',
  destructive: 'bg-danger text-white hover:bg-danger-300 hover:text-ink shadow-e1',
} as const;
  ...
  ({ variant = 'primary', size = 'md', loading, disabled, children, className = '', ...props }, ref) => (
    <motion.button
      ...
      className={`
        ...
        focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-oxblood
...
### 2.1 CQI Cup Score Badge — the signature atom
...
|---|---|---|---|---|
| Outstanding | 90.0–100 | `brass-300` / `ink-900` (7.32:1 AAA) | pill `radius-full` | 3 px ink-900 bean-groove tick left of numeral; "Outstanding" tooltip |
| Excellent | 85.0–89.9 | `oxblood-700` / white | pill | — |
| Very Good | 80.0–84.9 | `sage` (sage-600) / white | pill | — |
...
export function CupScoreBadge({ score, size = 'sm' }: { score: number; size?: 'sm' | 'md' }) {
  ...
  return (
    ...
    >
      {score >= 90 && <BeanGrooveTick />} {/* ink tick — earned brass */}
      {score.toFixed(1)}
    </span>
  );
}
...
| A/B Winner | `bg-brass text-ink` + `ease-seal` pop on reveal | `★ Winner` |
| Certified (FT/Organic/RA) | `bg-sage-100 text-sage border border-sage` | certification mark |
...
Geometry: h 40 px (`md`) / 48 px (`lg`), padding-x 16, `radius-md`, bg `surface`, 1 px `border-interactive` (3.40:1 ✓), text `base`/ink, placeholder `text-subtle`.

| State | Spec |
|---|---|
| Hover | border `neutral-700` |
| Focus | border `oxblood-700` + ring spec |
...
- Track: h 6 px, `radius-full`, bg `recessed`; filled portion `oxblood-700`.
- Thumb: 20 px circle, `surface` fill, 2 px `ink-900` border, shadow `e1`; hover shadow `e2`; focus ring spec; active scale 1.1 with `ease-seal`.
- Value readout: mono `lg`, `text-oxblood-800`, sits inside the overline label (mirrors `$12.00/lb` pattern).
...
- Box 16 px, `radius-sm`, 1.5 px `border-interactive`; checked fill `oxblood-700` with white check (9.09:1 AAA); focus ring spec.
- Toggle: 36×20 track, `neutral-500` off / `oxblood-700` on, 16 px knob; state also conveyed by position+label, never color alone.
- Indeterminate state for "select all" table rows: ink dash.
    ...
    <div className="space-y-1.5">
      ...
      <input
        ...
        className={`
          ...
          ${error ? 'border-danger' : 'border-neutral-600 hover:border-neutral-700 focus:border-oxblood'}
          focus:outline-none focus-visible:ring-2 focus-visible:ring-oxblood focus-visible:ring-offset-2 focus-visible:ring-offset-canvas
...
| Over-budget | container border becomes 2 px `warning`; badge `bg-warning-bg text-warning` (5.71:1 on tint AA ✓) |
...
| Metric bars | 4 columns (Cost/Quality/ESG/Logistics): caption label + mono % right; track h 6 px `bg-recessed radius-full`; fill `oxblood-700`, animates width once on mount, 500 ms `ease.out` |

### 4.2 States

- **Loading (skeleton):** same geometry in `recessed` blocks with 1.6 s paper shimmer (`paper-200 → paper-300`); never a bare spinner for card lists.
- **Over budget:** as above; excluded entirely when "Include out-of-budget lots" is off (architecture §4.2 `TOGGLE_OVER_BUDGET`).
- **Sold out (`available_quantity_lbs = 0`):** card at `opacity-45` except an ink "Position taken" stamp badge; CTA swaps to ghost "Join waitlist".
- **New arrival (≤ 14 days since `last_updated_at`):** 3 px `brass-500` leading edge (left border) — brass = earned attention.
- **Selected/comparison:** border `oxblood-700` 2 px + `bg-oxblood-100/40`.
  ...
  return (
    <motion.article
      ...
      className={`
        ...
        ${isNew ? 'border-l-[3px] border-l-brass' : ''}
      `}
...
| Body rows | h 48 px comfortable / 40 px compact; zebra `bg-surface` / `bg-paper-200`; row hover `bg-hover`; row focus-visible ring inset |
...
| Sort | header button with ▲/▼ (`text-oxblood-800` active, `text-subtle` inactive); `aria-sort` on `<th>` |
...
Lot (origin + varietal, truncated 24ch) · Process (badge) · Elevation (mono, m) · **Cup** (badge, default sort desc) · Price/lb (mono, `$`) · Available (mono, lbs, bar-under-text mini sparkbar for % of `total_production_lbs`) · ESG (mono %) · Certs (sage badges) · ETA (mono date) · Actions.
...
│  ink-900  ├───────────────────────────────────────────────┤
│  (inverse) │                                               │
│            │  View content on paper canvas             │
└────────────┴───────────────────────────────────────────────┘
```

### 6.2 Sidebar (`bg-ink-900 text-paper-50`)

- Brand slot: wordmark-only lockup (Playfair Display, paper) + "by Auctum" microline at 60% opacity (`text-[#A9A08C]`).
- Nav items: h 40 px, `radius-md`, 20 px icon + `text-sm` 500 label.
  - Default: `text-paper-50/70`; hover `bg-white/8 text-paper-50`.
  - Active: `bg-oxblood-700 text-white` (9.09:1 AAA ✓) + 3 px `brass-500` leading tick — the seal's accent marks the current section.
...
Breadcrumb (Inter 500, `text-muted`, current page `text-ink`) · global lot search (§3.1 search input, 320 px, collapses to icon button < md) · theme toggle (sun/moon, `aria-pressed`) · notifications (badge count in `danger` mono) · avatar menu (32 px ink monogram circle).
...
| Role | Value |
|---|---|
| Categorical series order | `ink-900` → `oxblood-700` → `brass-500` → `danger` → `sage` → `neutral-600` (replaces the ad-hoc array `['#8a8272','#2f6b4a','#4a6b8f','#a8721f']` in architecture §4.4) |
...
| Reference lines | budget ceiling = `warning` dashed; target = `oxblood-700` dotted |
| Empty chart | dashed `neutral-500` frame + "Awaiting telemetry" caption — never render axes without data |

### 7.2 KPI cards

`bg-surface rounded-lg p-5 shadow-e1`: overline label → `text-3xl` mono 700 value (Playfair Display only for the single hero KPI on a marketing page, never in ops dashboards) → delta line (▲ `text-success` / ▼ `text-danger` with mono % and "vs. prior period" caption). Sparkline h 32 px, `oxblood-700` 1.5 px stroke, no fill.

### 7.3 Dashboard-specific compositions

- **Campaign Intelligence (COF-001–005):** stepper of the 5 rules across the top (numbered medallions, active = ink fill, converted = brass); A/B results table per architecture §4.4 with `Winner` brass badge at probability ≥ 0.95, credible-interval bars as horizontal `oxblood-700` bars with whisker caps (error bars = 95% CI, `ink-900` 1.5 px); engagement line chart: opens = ink-900, clicks = oxblood-700, conversions = brass-500.
- **LTV/Churn (CRM):** churn-risk column uses a 5-stop sequential scale `sage → brass-300 → warning → danger` with the numeric score always shown (never color-only); survival-curve chart uses `danger` line with 95% band at 12% alpha.
- **Benchmarks (quantile regression):** peer band = `oxblood-100` area, median = `oxblood-800` line, "you" marker = 8 px `brass-500` diamond.
- **Inventory forecast:** actuals solid ink-900; Prophet/ARIMA forecast dashed `oxblood-700` with `oxblood-100` confidence cone.
...
- Component-level: skeletons that mirror final geometry (cards, rows, charts) in `bg-recessed` with paper shimmer; `aria-busy="true"` on the region; keep layout stable (no CLS).

### 8.2 Empty states (the only licensed wit)

Structure: 48 px Verification Seal illustration at `opacity-stamp`, `text-lg` Playfair Display headline, `text-sm text-muted` explainer, one primary/ghost action.
...
Full spec in `04-email-campaign-visual-system.md`. UI parity notes: the email header uses the ink band + paper seal lockup; email buttons mirror `Button primary` (oxblood, 6 px radius) rendered as bulletproof VML/anchor hybrids; merge tags render in-product as mono `oxblood-800` chips (`{sca_cup_score}`) in the template editor, and as italic serif fallbacks in plaintext parts. Dark-mode email fallbacks mirror §2.4 token values.
